...
alternative to categorical variables are quantitative variables, which were introduced 
...
In this chapter, we shall discuss techniques for identifying the presence, and for
...
A common method for visualising cross combinations of categorical data is to
use a bar chart.
...
#| fig-align: center
#| fig-cap: "Py: Barchart for Claritin data"
#| out-width: 80%
...
The colours in @fig-r-mosaic-pol reflect the sizes of the standardised residuals. We
shall discuss this in more detail in @sec-rc-tables.

The greater width of the tiles in the column for females reflects the larger 
marginal count for females. Compare this visualisation with @fig-r-barchart-pol.
...
```{python py-pol-4}
#| fig-align: center
#| fig-cap: "Py: Mosaic plot for political association data"
#| out-width: 80%
...
From both @fig-r-spine-heart and @fig-r-cd-heart, it is clear to see that
there is an increased proportion of death during follow-up associated with older
patients.
...
For the purpose of calculating $p$-values, a table is considered 'more extreme'
than the one observed, if its probability is less than or equal to that of the 
observed table. The $p$-value is the sum of all such probabilities.
...
Thus far, we have considered the situation concerning categorical variables
where each has only two possible outcomes (2x2 table). However, it is common
to want to check the association between two nominal variables where one of
them, or both, has more than 2 outcomes.
...
This sections covers bivariate measures of association for categorical variables.
...
-   A value of 1 indicates no association between $X$ and $Y$. If $X$ and $Y$
    were independent, this is precisely what we would observe.
-   Deviations from 1 indicate stronger association between the variables.
-   Note that, for odds ratios, deviations from 1 are not symmetric. For a given pair of
...
1.  Suppose we label the observed counts in the table as 
...
2.  Kendall $\tau_b$ is $$
    ...
    defining the categories. However, $\gamma$ has the advantage that it is more easily
    interpretable.
...
Also, take note of how log odds ratios, $\tau_b$ and $\gamma$ work. Firstly,
values close to 0 reflect weak association. Values of $a$ and $-a$ indicate the
same *strength*, but different direction of association. This allows the same
intuition that Pearson's correlation does. When you are presented with new
metrics, try to understand them by asking similar questions about them.
...
5.  The $\chi^2$ test we studied is a *test of independence*. It is a variant of
    ...
    a particular distribution. In our case, the presumed distribution
    is one where the variables are independent. Read more about the
...
1.  Use Python to create the spineplot for the heart failure data.
2.  Write a function in Python that converts a contingency table into "long" format.
3.  What is the interpretation of $\gamma$ implied by the text?
4.  From the political association data, extract the standardised residuals (under
