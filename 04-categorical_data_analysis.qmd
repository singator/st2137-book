...
A barchart can be created in Python with the following code. This barchart 
is slightly different from the one in @exm-pol-assoc-3 because it is not 
stacked.
...
The sum is taken over every cell in the table. Hence in a $2\times2$ table, as in @exm-chest-pain, 
...
*   Deviations from 0 indicate stronger association between the variables, 
    and deviations are now symmetric; a log-odds-ratio of -0.2 indicates the 
    same *strength* as 0.2, just the opposite direction.
