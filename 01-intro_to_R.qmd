...
Later on, S evolved into S-PLUS, which then became rather costly. **R**oss Ihaka
and **R**obert Gentleman from the University of Auckland decided to write a
stripped-down version of S. They named the new language R. Five years later,
version 1.0.0 of R was released on 29 Feb 2000. As of Dec 2025, the latest
version of R is 4.5.2 It is maintained by the [@rmanual].
...
::: callout-important
For our class, please ensure that you have version 4.5.1 or later. Functions in
older versions work differently, so you might face problems or differences with
some of the codes in the notes.
:::

![R: Base R GUI](figs/base_r_gui.PNG){#fig-base-R fig-alt="Base R GUI" fig-align="center"}
...
of times a year. Make sure you have at least version 2025.09.x for our course.

![R: Rstudio interface](figs/rstudio_gui.PNG){#fig-rstudio fig-alt="Rstudio GUI" fig-align="center"}
...
recycled to match the length of the longer one. This rule applies within all
...
these brackets to demarcate the row and column specifiers.
...
In R, dataframes are special kinds of lists. Hence, individual columns can be
retrieved from a dataframe (as a vector) using the `$` operator. These columns
can then be used to retrieve only the rows that satisfy certain conditions. In
order to achieve this task, we turn to logical vectors. The following code
returns only the rows corresponding to Gender equal to "M".

```{r r-access-3}
data2[data2$Gender == "M", ]
```

Logical vectors contain `TRUE`/`FALSE` values in their components. These vectors
can be combined with `&` (AND) and `|` (OR) operations to yield only the rows
that satisfy all conditions. Below, we return all rows where Gender is equal to
"M" and CA2 is greater than 85.
...
executing the instructions a fixed number of times.
...
beginning of each iteration. As long as it is TRUE, the block of R expressions
...
work, then do look up the help page (even before turning to [stackexchange](https://stackexchange.com/)). The
help pages within R are accessible even if you are offline.

To access the help page for a particular function, use the following command:
...
last check (Dec 2025), there are 23067 such packages. The packages can be
...
instance, the following command will install `stringr` (a package for string
...
1.  Use the `rep()` function to create the following vector:
```{r}
#| echo: false
rep(rep(1:3, c(3,2,1)), 2)
```

2.  What do the functions `saveRDS()` and `readRDS()` do?

3.  From the `data2` dataframe in @sec-01-read-data, return all rows where
    either `Gender` is equal to "M", or `HW` is "A".
    
4.  Create a list object that, when printed, returns:
```{r}
#| echo: false
...
5.  Why does this line of code return an error? When should `&&` be used?
```{r}
#| eval: false
data2[data2$Gender == "M" && data2$CA2 > 85, ]
```
