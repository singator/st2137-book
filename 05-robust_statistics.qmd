...
Introductory statistics courses describe and discuss inferential methods based
on the assumption that data is Normally distributed. However, we never truly
know the distribution from which our data has arisen. The sample itself may
deviate from Normality in various ways. To begin with, we might observe that the
data has heavier tails than a Normal distribution. Second, the data could
suggest that the originating distribution is heavily skewed, unlike the
symmetric Normal.
...
In this chapter, we shall introduce the concept of robustness and estimators of
location and scale that have this property. Although we only touch on basic
statistics in this topic, take note that robust techniques exist for regression
and ANOVA as well. It is a vastly under-used technique.
...
$$
    We denote the sample median as $X_{(1/2)}$.
    
## Datasets

For this topic, we shall work with a couple of datasets that are clearly not
Normal.
...
of an analytical chemistry experiment -- the amount of copper ($\mu g$) 
...
statistic is clearly being affected by the largest two values. This topic is
about techniques that will work well even in the presence of such large
anomalous values.

:::

The second dataset is from a textbook on robust estimation techniques.
...
The choice of $\rho$ confers certain properties on the 
resulting estimator. For instance, $\psi = \rho'$
is referred to as the *influence function*, which measures the relative change in a
statistic as a new observation is added. To find the $\hat{\mu}$ that minimises
...
2. Replace the smallest $g$ values in the sample with $X_{(g)}$ and the largest 
   $g$ values with $X_{(n-g+1)}$.
3. Compute the arithmetic mean of the resulting $n$ values.
$$
X_w = \frac{g\cdot X_{(g)} + X_{(g+1)} + \cdots + X_{(n-g)} + g \cdot X_{(n-g+1)}}{n}
$$
...
2. [Scipy stats](https://docs.scipy.org/doc/scipy/reference/stats.html#statsrefmanual):
   ...
   and `mstats.winsorize()` methods in Python.
   
   
## Exercises

1.  List two ways in which data could possibly deviate from Normality.
2.  Compute the trimmed mean for the `G3` variable in the student performance 
    dataset, using $\gamma=0.1$.
3.  Winsorise the `chem` dataset, with $\gamma=0.1$, and create QQ-plots for the 
    original and new dataset.
4.  Identify the error(s) in the following R implementation of MAD, and create 
    a Python function with the correct algorithm.
```{r}
#| eval: false
mad2 <- function(x) {
...
5.  Gini's mean difference is defined to be the mean absolute difference between
    any two distinct elements of a vector. Write a function that implements it, and 
    compare it with robust measures of spread that were discussed in this chapter.