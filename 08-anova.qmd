...
In this topic, we introduce the *one-way analysis of variance* (ANOVA). It
generalises the $t$-test methodology to more than 2 groups. Hypothesis tests in
the ANOVA framework require the assumption of Normality. When this does not
hold, we turn to the Kruskal-Wallis test - a non-parametric version of ANOVA, to
...
specific comparisons between groups as well. This chapter also covers how we can
construct confidence intervals in those cases.
...
The following example is taken from @ekstrom2015statistical. An experiment with
dung from heifers[^08-heifers] was carried out in order to explore the influence
of antibiotics on the decomposition of dung organic material. As part of the
experiment, 36 heifers were randomly assigned into six groups.
...
groups. The remaining group served as a control group. A bag of dung from each 
heifer was dug into the soil, and after 8 weeks, the amount of organic material 
remaining in each bag was measured.
...
distribution with mean 0 and variance $\sigma^2$. This leads to the following
...
2. Setting the reference level $\alpha_1= 0$.
...
3. *The variance within each group is the same.* In ANOVA, the $MS_W$ is a
   pooled estimate (across the groups) that is used; in order for this to be valid,
   the variance within each group should be identical. Just as in the 2-sample
   situation, we shall avoid separate hypotheses tests and proceed with the
   rule-of-thumb that if the ratio of the largest to smallest standard deviation is
   less than 2, we can proceed with the analysis.
   

::: {style="background-color: #D5D1D1; padding: 20px" #exm-antibiotics-2}

### F-test for Heifers Data
\index{Heifers!F-test}

We begin by applying the overall $F$-test to the heifers data, to assess if 
...
![SAS: F-test for heifers data](figs/sas_anova_f_test.png){width=70% fig-align="center"}
...
![SAS: Group summaries for heifers data](figs/sas_anova_heifer_group_means.png){width=70% fig-align="center"}
...
To check the assumptions, we can use the following code, which results in the 
two plots @fig-r-heifers-normality-1 and @fig-r-heifers-normality-2.
...
```{python py-normality}
#| fig-cap: "Py: Normality checks for heifers"
# Python
...
Let us attempt to answer question (2) from the set of questions earlier in
@exm-antibiotics-1.
...
In order to get SAS to generate the estimate, modify the default ANOVA code to
include `clparm` in the `model` statement, and include the `estimate` statement.
...
Let sub-group (A) consist of Ivermectin and Fenbendazole. 
...
![SAS: Contrast estimation for heifers data](figs/sas_anova_heifer_contrast2.png){width=70% fig-align="center"}
...
of each test at $\alpha$, then we should perform each of the $m$ tests or confidence 
...
![SAS: Multiple comparisons in heifers data](figs/sas_anova_heifer_hsd.png){width=70% fig-align="center"}
...
![SAS: Kruskal-Wallis output, heifers data](figs/sas_kruskal_wallis_a.png){width=70% fig-align="center"}
...
The purpose of this topic was to introduce you to the one-way ANOVA model. While
there are restrictive distributional assumptions that it entails, I once again
urge you to focus on the information the method conveys. It attempts to compare
the within-group variance to the between-group variance. Try to avoid viewing
statistical procedures as flowcharts. If an assumption does not hold, or a
p-value is borderline significant, or try to identify which points lead to the
result being significant (or not).
...
3. [Contrast coding](https://stats.oarc.ucla.edu/r/library/r-library-contrast-coding-systems-for-categorical-variables/): More information about how to set the constraints
   for estimation in R.
4. [Type I,II,III SS](https://rcompanion.org/rcompanion/d_04.html): When working with
   ANOVA (especially with unbalanced models), you may come across different types of
   Sum of Squares. You will learn more about this in ST4233.


### Exercises

1.  Compare the formula for confidence intervals in @sec-spec-grps with the 
    formula in @sec-t-test-formulas. How are they different?
2.  Consider the formula for test statistic in the F-test. Can you conceive of a 
    robust version of it?
3.  Assess if the average of (Alfacyp, Enroflox, Spiramycin) is significantly 
    different from (Ivermectin, Fenbendazole) at 5% significance level.
4.  Apply the Bonferroni correction to the following two comparisons:
    1.  Alfacyp vs. Ivermectin
    2.  Enrofloxacin vs. Fenbendazonle
5.  Apply the ANOVA F-test to the `G3` scores, by `Medu`, for the student performance
    dataset.